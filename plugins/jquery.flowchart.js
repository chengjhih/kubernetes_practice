...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _addPodClass: function (operatorId) {
            this.data.operators[operatorId].internal.els.operator.addClass('pod');
        },

        _getOperatorFullElement: function (operatorData) {
            var infos = this.getOperatorCompleteData(operatorData);
            var operatorType = infos.title.split(" ")[0];
            ...
            // custom for mount arrow
            var $div_operator_mount = $('<div style="position: absolute; bottom: -50px;" class="flowchart-operator-inputs-outputs"></div>');
            $div_operator_mount.appendTo($operator);
            var $operator_mount = $('<div class="flowchart-operator-volume-mount"></div>');
            $operator_mount.appendTo($div_operator_mount);


            if (operatorType === "Pod") {
                $operator.addClass('pod');
                $operator_title.addClass('pod');
            } else if (operatorType === "AzureKeyVaultSecret") {
                $operator.addClass('azurekeyvaultsecret'); 
                $operator_title.addClass('azurekeyvaultsecret');    
            } else if (operatorType === "Certificate") {
                $operator.addClass('certificate'); 
                $operator_title.addClass('certificate');
            } else if (operatorType === "ClusterIssuer") {
                $operator.addClass('clusterissuer'); 
                $operator_title.addClass('clusterissuer');
            } else if (operatorType === "Secret") {
                $operator.addClass('secret'); 
                $operator_title.addClass('secret');
            } else if (operatorType === "ConfigMap") {
                $operator.addClass('configmap'); 
                $operator_title.addClass('configmap');
            } else if (operatorType === "Image") {
                $operator.addClass('image'); 
                $operator_title.addClass('image');
            } else if (operatorType === "Container") {
                $operator.addClass('container'); 
                $operator_title.addClass('container');
                $operator_inputs.addClass('container');
            } else if (operatorType === "Deployment") {
                $operator.addClass('deployment'); 
                $operator_title.addClass('deployment');
                $operator_inputs.addClass('deployment');
                $operator_outputs.addClass('deployment');
            } else if (operatorType === "Service") {
                $operator.addClass('service'); 
                $operator_title.addClass('service');
                $operator_inputs.addClass('service');
                $operator_outputs.addClass('service');
            } else if (operatorType === "Ingress") {
                $operator.addClass('ingress'); 
                $operator_title.addClass('ingress');
                $operator_inputs.addClass('ingress');
                $operator_outputs.addClass('ingress');
            } else if (operatorType === "PersistentVolume") {
                $operator.addClass('persistentvolume'); 
                $operator_title.addClass('persistentvolume');
            } else if (operatorType === "PersistentVolumeClaim") {
                $operator.addClass('persistentvolumeclaim'); 
                $operator_title.addClass('persistentvolumeclaim');
            } else if (operatorType === "Volume") {
                $operator.addClass('volume'); 
                $operator_title.addClass('volume');
            }
            ...
            for (var key_o in infos.outputs) {
                if (infos.outputs.hasOwnProperty(key_o)) {
                    if ("isMount" in infos.outputs[key_o]) {
                        addConnector(key_o, infos.outputs[key_o], $operator_mount, 'outputs');
                    } else {
